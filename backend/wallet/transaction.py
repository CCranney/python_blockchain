...
class Transaction:
    ...
    def from_json(transaction_json):
        """
        Deserializes a transaction's json representation back to a transaction.
        """
    ...
    def is_valid_transaction(transaction):
        ...
        if transaction.input == MINING_REWARD_INPUT:
            if list(transaction.output.values()) != [MINING_REWARD]: raise Exception('Invalid mining reward')
            return
    ...
    def reward_transaction(miner_wallet):
        """
        Generate a reward transaction to award the miner.
        """
        output = {}
